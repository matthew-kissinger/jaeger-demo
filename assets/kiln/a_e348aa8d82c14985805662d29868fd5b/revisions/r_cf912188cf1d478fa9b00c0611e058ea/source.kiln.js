...
const SITE = {
  version: 1, seed: 203, units: 'metres', deck: { min: [-240, -6, -200], max: [720, 0, 200] },
  pilotBounds: { min: [-145, 0, -180], max: [680, 58, 180] },
  flightBounds: { min: [-700, 0, -500], max: [900, 200, 500] },
  dock: [-155, 0, 0], startSourceDisplacement: [52, 0, 0], cradleClip: 'CradleRelease',
...
async function build() {
  ...
  const concreteMap = noise('site.concrete.albedo', 0x424d50, 0x4a5356, 12, 203);
  const rockMap = noise('site.rock.albedo', 0x303a3b, 0x555c56, 5, 62);
  const paintMap = noise('site.paint.albedo', 0x21323d, 0x253843, 25, 91);
  ...
  for (const sign of [-1, 1]) {
    ...
    for (let x = -154; x < 200; x += 16) box('LaunchLaneDash', [6, 0.024, 0.3], [x, 0.018, sign * 37], white, deck);
  ...
  for (const z of [-49, 49]) { box('PortalUpright', [3, 88, 3], [12, 44, z], ochre, hangar); box('DoorLight', [0.15, 72, 0.4], [13.55, 47, z], cyan, hangar); }
  ...
  for (const sign of [-1, 1]) {
    ...
    box('DoorHeader', [0.2, 0.12, 1.2], [11.25, 2.15, sign * 58], cyan, hangar);
    box('ServiceAwning', [3, 0.16, 6], [12.2, 3, sign * 58], ochre, hangar);
  }
  const cradle = createPivot('Cradle', [-155, 0, 0], root);
  for (const sign of [-1, 1]) {
    await bevel('PierFoot', [19, 3.1, 18], [-16, 1.45, sign * 35], concrete, cradle, 0.6);
    ...
    for (let y = 7; y < 98; y += 14) {
      box('TowerStiffener', [7.6, 0.78, 9.6], [-16, y, sign * 35], silver, cradle);
    }
    box('LiftGuide', [0.9, 90, 0.9], [-12.1, 47, sign * 35], ochre, cradle);
    for (const y of [18, 42, 69, 92]) {
      box('AccessCatwalk', [9, 0.35, 10.4], [-16, y, sign * 35], dark, cradle);
      // Real one-metre rail at maintenance levels.
      for (let z = -4; z <= 4; z += 2) cylinder('RailPost', 0.05, 1.05, [-11.7, y + 0.67, sign * 35 + z], silver, cradle, [0, 0, 0], 6);
      beam('HandRail', [-11.7, y + 1.225, sign * 31], [-11.7, y + 1.225, sign * 39], 0.06, 0.06, silver, cradle);
      box('CatwalkBeacon', [0.3, 0.3, 1.5], [-11.4, y + 0.27, sign * 35], amber, cradle);
    }
    // Fixed cantilever rails support the carriage throughout its 20m release travel.
    box('CarriageRail', [8, 0.76, 34], [-7, 46.58, sign * 43], dark, cradle);
    beam('RailCantilever', [-16, 40, sign * 35], [-7, 46.6, sign * 53], 1.8, 1.8, steel, cradle);
    beam('RailBrace', [-16, 43, sign * 35], [-7, 46.6, sign * 28], 1.2, 1.2, steel, cradle);
  ...
  box('RearMast', [4, 66.1, 7], [-33, 32.95, 0], steel, cradle);
  ...
  for (const target of SITE.targets) {
    const t = createPivot(target.id, target.position, root), h = target.center[1], r = target.radius, s = r / 13;
    await bevel('TargetBase', [22, 3.1, 22], [0, 1.45, 0], concrete, t, 0.5);
    box('TargetMast', [4.12, h - 2, 5], [0.1, (h - 2) / 2 + 2, 0], steel, t);
    for (const z of [-6.5, 6.5]) beam('TargetSupport', [0, 2, z], [0, h - 13, z * 0.22], 1.2, 1.2, ochre, t);
    const disk = cylinder('TargetShield', r, 4, [0, h, 0], dark, t, [0, 0, 90], 24);
    cylinder('TargetPlate', 11.6 * s, 0.6, [-2.35, h, 0], steel, t, [0, 0, 90], 24);
    const face = cylinder('TargetRing', 7.4 * s, 0.64, [-2.7, h, 0], ochre, t, [0, 0, 90], 24);
    cylinder('TargetInset', 6.3 * s, 0.7, [-3.06, h, 0], dark, t, [0, 0, 90], 24);
    cylinder('TargetCore', 2.1 * s, 0.74, [-3.45, h, 0], cyan, t, [0, 0, 90], 16);
    for (const sign of [-1, 1]) {
      box('TargetTick', [0.1, 1.5, 4], [-2.7, h + sign * 9.5 * s, 0], white, t);
      box('TargetTick', [0.1, 4, 1.5], [-2.7, h, sign * 9.5 * s], white, t);
    }
  ...
  for (const s of [-1, 1]) {
    ...
    box('DoorHandle', [0.25, 0.05, 0.04], [0.92, 1.65, s * 1.115], silver, truck);
    box('Mirror', [0.12, 0.25, 0.1], [2.15, 2.15, s * 1.38], dark, truck);
    box('MirrorArm', [0.05, 0.05, 0.32], [2.15, 2.08, s * 1.23], silver, truck);
    ...
    box('Headlight', [0.1, 0.2, 0.4], [2.395, 1.45, s * 0.7], white, truck);
  }
  ...
  for (let i = 0; i < 5; i++) {
    ...
    part('EquipmentCase', projected(crateGeo, 2), steel, p, stores);
  ...
  terrain('WesternBluff', -750, -238, -590, 590, 48, 72, (x, z) => {
    ...
    const farEdge = Math.max(0, Math.min(1, (x + 750) / 135));
    return -9 + farEdge * inland * end * (35 + Math.abs(ridge)) + 2 * farEdge * inland * Math.sin(x * 0.093 + z * 0.075);
  });
